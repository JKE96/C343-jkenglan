...
public class KDtree<Key extends Comparable<?super Key>, E> {
	...
	//helper function that checks if distances are within range and returns true if in range r
	public boolean compareDist(Key[] first, Key[] second, int r){
		...
		double ans = Math.sqrt(a); //square roots to finish the distance operation ans is the distance between first and second keys
		if (ans < r){
			return true;
		}
		return false;
