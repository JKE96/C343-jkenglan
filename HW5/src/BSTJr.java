...
public class BSTJr <K extends Comparable<?super K>> {
	...
	private K getKSmallest(BinNode<K> bn, int a){
		
		if (bn.getLeft()==null || bn.getRight()==null){
			return bn.getKey();
		}
		BinNode left = bn.getLeft();
		BinNode right = bn.getRight();
		if (a ==  left.getSize() +1){
			return  bn.getKey();
		}
		else if(a> left.getSize()+1){
			return getKSmallest(right, a- (bn.getSize()+1) );
		}
		else {
			return getKSmallest(left, a);
		}
	...
	public static void main(String[] argv) {
		...
		BinNode one = new BinNode(20);
		one.setSize(3);
		BinNode two = new BinNode(10);
		two.setSize(1);
		...
		System.out.println(tree.getKSmallest(one, 3));
	}
}